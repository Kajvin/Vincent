...
class CVincent implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath         = VINCENT_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = VINCENT_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl        = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $wi->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $wi
    ...
    $wi = &$this;
    // First the default Vincent themes/functions.php
  ...
  /**
   * Draw HTML for a menu defined in $wi->config['menus'].
   *
