...
<a href="http://validator.w3.org/checklink?uri={$wi->request->current_url}">links</a>
<a href="http://qa-dev.w3.org/i18n-checker/index?async=false&amp;docAddr={$wi->request->current_url}">i18n</a>
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CVincent::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CVincent::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CVincent::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CVincent::Instance()->themeParentUrl . "/{$url}");
}
...
function render_views($region='default') {
  return CVincent::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CVincent::Instance()->views->RegionHasView(func_get_args());
}